...
describe("handleWorkerReserved", () => {
  ...
  it("no-op when worker not in registry", async () => {
    const { wss } = makeDeps(taskManager);
    await expect(wss.handleWorkerReserved("unknown-worker")).resolves.toBeUndefined();
  });
});
