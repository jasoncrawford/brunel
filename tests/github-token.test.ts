...
describe("GithubToken.resolve", () => {
  it("returns config token without calling gh CLI (highest priority)", async () => {
    const token = await new GithubToken({ githubToken: "config-token" }).resolve();
    expect(token).toBe("config-token");
    expect(mockExecFile).not.toHaveBeenCalled();
  });

  it("falls back to gh CLI when config token is absent", async () => {
  ...
  it("returns null when config token absent and gh CLI fails", async () => {
    mockExecFile.mockImplementationOnce((_cmd: string, _args: string[], cb: (...a: unknown[]) => void) => {
  ...
  it("returns null when config token absent and gh is not installed (ENOENT)", async () => {
  ...
  it("returns null when config token absent and CLI output is empty", async () => {
    mockExecFile.mockImplementationOnce((_cmd: string, _args: string[], cb: (...a: unknown[]) => void) => {
  ...
  it("returns null when no config token and no gh CLI", async () => {
    mockExecFile.mockImplementationOnce((_cmd: string, _args: string[], cb: (...a: unknown[]) => void) => {
