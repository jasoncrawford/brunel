 ...
 */
export function pick(options: string[]): Promise<number>;
export function pick(options: string[], config: PickConfig): Promise<PickResult>;
export function pick(options: string[], config?: PickConfig): Promise<number | PickResult> {
 ...
 */
export function pickMultiple(options: string[], promptStr?: string): Promise<number[]> {
  return new Promise((resolve) => {
    let idx = 0;
    let done = false;
    const count = options.length;
