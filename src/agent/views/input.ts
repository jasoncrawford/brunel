  ...
  pick(options: string[], config?: PickConfig): Promise<number | PickResult> {
    ...
    return new Promise((resolve) => {
      ...
      function renderLine(i: number): string {
        ...
        return Input.pickerLine(text, i === idx, marker);
      }
  ...
  pickMultiple(options: string[], promptStr?: string): Promise<number[]> {
    return new Promise((resolve) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        process.stdout.write(Input.pickerLine(`${check} ${options[i]}`, i === idx) + "\n");
      }
      ...
      function redraw() {
        ...
        for (let i = 0; i < count; i++) {
          const check = selected.has(i) ? "◉" : "○";
          process.stdout.write(Input.pickerLine(`${check} ${options[i]}`, i === idx) + "\x1b[K\r\n");
        }
      }
    ...
    return new Promise((resolve) => {
      ...
      function renderLine(i: number): string {
        ...
        if (i === idx) {
          ...
          return Input.pickerLine(`${numStr}. ${text}`, true);
        } else {
          ...
          return Input.pickerLine(`${numStr}. ${text}`, false);
        }
      }
