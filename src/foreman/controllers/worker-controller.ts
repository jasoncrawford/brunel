...
export class WorkerController {
  ...
  async handleWorkerHello(workerId: string, ws: WebSocket, msg: Extract<Wire.WorkerMessage, { type: "worker_hello" }>): Promise<void> {
    ...
    if (msg.status === "revive") {
      await this.handleReviveHello(workerId, ws, repo, versionInfo);
  ...
  /**
   * Handles the "revive" branch of a worker_hello — a new process is taking
   ...
   */
  async handleReviveHello(workerId: string, ws: WebSocket, repo: Repo, versionInfo?: { version?: string; protocolVersion?: number }): Promise<void> {
    try {
      // Fatal if the worker is currently connected
      if (Worker.fromRegistry(workerId)) {
        this._workerLog(workerId, "revive rejected — worker is currently connected");
        this.messenger.sendError(ws, `Worker ${workerId} is currently connected — cannot revive`, true, workerId, repo.id);
        return;
      }
      ...
      if (!dbWorker) {
        this._workerLog(workerId, "revive rejected — no DB record found");
        this.messenger.sendError(ws, `No worker found with ID ${workerId} — cannot revive`, true, workerId, repo.id);
        return;
      }

      // Fatal if the connecting repo doesn't match the worker's recorded repo
      if (dbWorker.repoId !== repo.id) {
        this._workerLog(workerId, `revive rejected — repo mismatch (worker repo_id=${dbWorker.repoId}, connecting repo_id=${repo.id})`);
        this.messenger.sendError(ws, `Worker ${workerId} belongs to a different repo — cannot revive as ${repo.fullName}`, true, workerId, repo.id);
        return;
      }
      ...
      if (task && task.status !== "complete") {
        this._workerLog(workerId, `revive: reclaiming active task #${task.issueNumber}`);
        const worker = Worker.register(workerId, ws, repo, versionInfo);
        await this.reclaimWorker(worker, task);
        // reclaimWorker sends hello_ack { status: "assigned" }.
        // Also send task_assigned so the reviving worker learns its task details.
        const baseSeqId = await WebhookEvent.currentMaxId();
        this.messenger.send(worker, {
          type: "task_assigned",
          taskId: task.taskId,
        ...
        this._workerLog(workerId, `revive → task_assigned #${task.issueNumber}`);
        return;
      }
      ...
      if (!worker) return;
      this._workerLog(workerId, "revive: no active task, registering fresh");
      this.messenger.send(worker, { type: "hello_ack", workerId: worker.workerId, status: "ready", repoStatus: repo.status });
    } catch (err) {
      log(`ERROR handleReviveHello ${workerId}: ${fmtError(err)}`);
      this.messenger.sendError(ws, `Internal error during revive: ${fmtError(err)}`, false, workerId, repo.id);
    }
  }
