...
function startDepsLoad(issueNumber: number, body: string, deps: RoutingDeps): void {
  const { taskManager, repo, token, githubApiUrl, flog } = deps;
  Task.fetchBlockers(issueNumber, body, { repo, token, apiUrl: githubApiUrl })
    .then(async (blockers) => {
      ...
      await taskManager.assignIdleWorkers();
    })
...
export async function routeIssueEvent(
  ...
  const { taskManager, flog, repo, taskLabel } = deps;
...
export class ForemanWss {
  ...
  constructor({ config, server, taskManager, adminWss }: ForemanWssOptions) {
    ...
    // Listen for task assignment outcomes from TaskManager.
    taskManager.on("assigned", ({ task, queued, workerId: wid }: { task: Task; queued: WebhookEvent[]; workerId: string }) => {
      ...
      sendMsg(wid, assignMsg);
      log(wid, `→ task_assigned #${task.issueNumber} "${task.title}"`);
      for (const evt of queued) {
        const evtMsg: Wire.ForemanMessage = { type: "event_notification", taskId: task.taskId, event: evt.toWorkerPayload() };
        sendMsg(wid, evtMsg);
        log(wid, `→ event_notification #${task.issueNumber} ${evt.eventName} (queued)`);
      }
    });

    taskManager.on("assign_failed", ({ workerId: wid, err }: { workerId: string; err: unknown }) => {
      flog(`ERROR Failed to persist assignment: ${fmtError(err)}`);
      log(wid, "→ idle (DB write failed)");
    });

    const routingDeps: RoutingDeps = { taskManager, repo, token, githubApiUrl, taskLabel, sendMsg, flog };
    ...
    this.reconcile = () => taskManager.assignIdleWorkers();
