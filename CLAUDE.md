...
A unified REPL + worker loop. `AgentController` in `controllers/agent-controller.ts` owns the query loop; `index.ts` is a thin composition root (~30 lines) that constructs objects and calls `agentController.start()`.
