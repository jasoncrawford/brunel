...
- `src/` root — `config.ts` (unified config loader), `types.ts` (shared wire types: `ForWorkerMsg` for foreman→worker messages, `WorkerEvent` for event payloads delivered to workers; previously called `ForemanMessage`/`GitHubEvent`), `utils.ts`
