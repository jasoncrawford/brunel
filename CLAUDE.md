...
- **`src/agent/index.ts`** — Entry point: unified agent loop for both REPL and worker modes. `main()` runs the input loop and owns command dispatch, signal handling, and workspace lifecycle. `startWorkerMode()` (exported for tests) sets up the workspace and `WorkerSession` and calls `main()` with a `workerMode` parameter. Interactive REPL (default) or worker process (`--worker-mode`).
...
- **`src/agent/worker.ts`** — WebSocket client and task lifecycle only. `WorkerSession` owns the WebSocket connection, foreman protocol (hello/ack/task_assigned/event_notification/task_complete/goodbye), task state, and `WorkerStatusModel`. Exposes `runForQuery(prompt)`, `completeCurrentTask()`, `clearSession()`, and `workspaceCommandDeps` for use by `main()`. No command dispatch, signal handling, or input loop. `WS_TASK_ASSIGNED` and `WS_EVENT` are exported sentinels used by the main loop. Also exports `classifyEvent`, `debounceMs`, `WorkerStatusModel`.
