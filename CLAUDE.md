...
Other key files: `index.ts` (entrypoint; constructs `settings`, `statusBar`, and `display` in order at startup — `const statusBar = new StatusBar({ agentId, settings })`, `const display = new Display(config, statusBar)` — and passes them to all controllers; `runQuery(display, permConfig, ...)` and `main(runQueryFn, permConfig, display, settings, ...)` receive display and settings as parameters), `worker-prompts.ts` (prompt templates).
