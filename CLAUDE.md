...
- **Controllers** (`controllers/`) — `http-server.ts` handles webhooks + REST + SPA. `wss.ts` (`ForemanWss`) is the WebSocket protocol layer for worker↔foreman communication; it delegates installation webhook events to `InstallationsController`. `admin-ws.ts` broadcasts to the admin dashboard. `installations-controller.ts` (`InstallationsController`) handles GitHub App installation lifecycle: `installation.created/deleted` and `installation_repositories.added/removed` — creates/deletes `Installation` records, links/unlinks repos via `Repo.linkInstallation()`/`Repo.unlink()`, activates/deactivates repos, and seeds tasks via `TaskManager.loadIssuesFromGithub(installationGithubId)`.
- **Clients** (`clients/`) — `db-client.ts` wires the shared Supabase client. `github.ts` wraps GitHub API calls. `GithubClient` supports two auth modes: personal-token (reads `githubToken` from config) and App installation token (pass `installationGithubId` to the constructor; `resolveToken()` auto-mints a short-lived token via `mintInstallationToken()`). Callers that have an `Installation` record should always pass `installation.githubId` to the constructor.
