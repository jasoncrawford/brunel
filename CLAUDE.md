...
- **Browser tests**: the server is shared across all tests — use unique issue numbers rather than resetting server state. Test helper endpoints (`/test/*`) must await async side effects before responding. For features that read `foreman_messages`, mock the API response rather than relying on real DB rows.
