...
- **`src/foreman/task-model.ts`** — `TaskModel` class and `Task` interface: the single owner of all task state. Maintains an in-memory cache (for sync lookups in hot event-routing paths and ephemeral data like event queues) alongside a persistent `TaskStore` (Supabase); every state transition updates both atomically. Owns `labeledIssues`/`openIssues` internally (creates them itself). Provides startup methods: `loadActiveTasksFromDb()` and `loadIssuesFromGithub()`.
...
- `TaskModel` owns both the in-memory task cache and the persistent store (`TaskStore`/Supabase). **The DB is the authoritative source of truth; in-memory state is a derived cache.**
