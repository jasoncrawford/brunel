...
Key files: `worker.ts` (WS protocol + task lifecycle; also owns `generateAgentId` and `WORKER_NAMES`; `startWorkerMode()` takes no arguments and reads from `getConfig()` directly), `display.ts` (TUI rendering: colors, print helpers, diff/markdown rendering; `setVerbose()`/`setThinkOutLoud()` mutate the config singleton so verbose state persists across the whole process), `status-bar.ts` (status bar state and rendering; exports `StatusBar` class — holds all worker status state — and a shared `statusBar` singleton initialized via `initStatusBar()` at startup; callers import directly from this module, not via `display.ts`), `settings.ts` (runtime-settable preferences; exports `Settings` class — owns model and effort state, emits `"change"` on updates so `statusBar` can subscribe reactively; also exports effort/model constants, types, and model-cache helpers), `command-registry.ts` (`CommandRegistry` class — instantiated in `index.ts` and injected into all modules; supports scoped sub-registries via `scoped(prefix)`; `registry.listCommandNames()` and `registry.listCommands()` return all available commands including file-based commands and skills), `workspace.ts` (git/npm workspace management), `worker-prompts.ts` (prompt templates).
