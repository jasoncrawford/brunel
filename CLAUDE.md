...
The `/worker:revive <prefix>` command lets a new worker process take over a dead worker's identity. It finds the workspace directory whose name starts with `prefix` (error if none or ambiguous), sets `agentStatus.agentId` to the dead worker's ID, attaches to the existing workspace directory (validates `.git`, re-applies auth, writes lockfile), and connects with `status: "revive"`. The foreman validates: fatal error if the worker is currently connected, if no DB record exists, or if the repo doesn't match. If the dead worker had an active task, the foreman sends both `hello_ack { assigned }` and `task_assigned` so the new process resumes normally. If no active task, it sends `hello_ack { ready }`. The worker also attempts to locate the dead worker's last Claude session ID (newest `.jsonl` in `~/.claude/projects/`) so conversation history is preserved.
