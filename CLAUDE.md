...
- **Keyboard rules**: `^C`/`^D` at a visible prompt (states 1 or 3-at-prompt) = quit — calls `stop()` (with task-quit confirmation if active) then exits on `^D`/`/exit`. `^C` with no visible prompt (state 3 running a query) = interrupt. `^D` with no visible prompt (state 2, `ask("")`) = no-op.
