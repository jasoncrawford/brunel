...
- `buildMessageSummary(direction, msgType, taskId, payload)` in `src/db.ts` is the single source of truth for log entry summaries. It is used by both the real-time broadcast path (`broadcastMessageEvent` in `foreman.ts`) and the DB read path (`messageToEntry` in `db.ts`). Add new message type formatting here rather than in both places.
