...
- **Browser tests**: the server is shared across all tests — use unique issue numbers rather than resetting server state. Test helper endpoints (`/test/*`) must await async side effects before responding. The in-memory DB shim (`tests/helpers/memory-db.ts`) is generic — it works for any table and needs no changes when the schema evolves. When adding a new table with `NOT NULL DEFAULT` columns that models read, add an entry to `columnDefaultFns` in `memory-db.ts` so the shim mirrors those defaults.
