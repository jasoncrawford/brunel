...
A unified REPL + worker loop. `main()` in `index.ts` runs both interactive and worker modes; `startWorkerMode()` in `controllers/worker.ts` sets up the WebSocket session and returns before `main()` takes over the query loop.

Follows MVC with three subdirectories:

- **Models** (`models/`) — `workspace.ts` (git/npm workspace management; extends `EventEmitter`, emits `"clone-start"`, `"reset-start"`, `"reset-retry"`, `"reset-reclone"`, `"npm-install"`, `"destroy"` events instead of calling `print()` directly — callers subscribe in `startWorkerMode()`), `settings.ts` (runtime-settable preferences; exports `Settings` class — owns model and effort state, emits `"change"` on updates so `statusBar` can subscribe reactively; also exports effort/model constants, types, and model-cache helpers), `query-stats.ts` (tracks token usage and turn counts during a query run)
- **Views** (`views/`) — `display.ts` (TUI rendering; exports `Display` class with `BrunelConfig` injected in constructor — class methods use `this.config.verbose`; also exports a shared `display` singleton initialized via `initDisplay()`, standalone delegate functions `print`/`printBlock`/`printMessage`/`printForemanMessage`, and all pure utilities like `c`, `s`, `resolve`, `fmtEvent`, etc.), `status-bar.ts` (status bar state and rendering; exports `StatusBar` class — holds all worker status state — and a shared `statusBar` singleton initialized via `initStatusBar()` at startup), `input.ts` (readline-based REPL input: `ask`, `pick`, `pickMultiple`, `pickQuestion`)
- **Controllers** (`controllers/`) — `worker.ts` (WS protocol + task lifecycle; owns `WorkerSession`, `generateAgentId`, `WORKER_NAMES`, `startWorkerMode()`, `registerWorkerCommands()`; `startWorkerMode()` takes no arguments and reads from `getConfig()` directly), `workspace-commands.ts` (registers `/workspace:*` slash commands; imports `print`/`c` standalone delegates from `views/display.ts` so vitest mocking works), `command-registry.ts` (`CommandRegistry` class — instantiated in `index.ts` and injected into all modules; supports scoped sub-registries via `scoped(prefix)`; `registry.listCommandNames()` and `registry.listCommands()` return all available commands including file-based commands and skills)

Other key files: `index.ts` (entrypoint; calls `initDisplay(new Display(config))` and `initStatusBar(new StatusBar(...))` at startup), `worker-prompts.ts` (prompt templates).
...
- Use `print()` (the standalone delegate from `views/display.ts`, not `console.log`) for output in agent/worker code — it routes through the status-bar-aware renderer so messages don't corrupt the TUI. Import as `import { print, c } from "../views/display.js"` (or adjust the relative path). Do not call `display.print()` directly — use the standalone function so vitest module mocking works.
