...
- `Task` (active-record in `task.ts`) owns all DB reads/writes. `TaskManager` (`task-model.ts`) owns only ephemeral state (event queues, branch maps, issue tracking). **There is no in-memory cache — all reads go through `Task` statics.** `initDb(supabase)` from `db-client.ts` must be called once at startup to wire the DB; `Task.events.on("changed", ...)` wires any observers. Supabase is required to run the foreman; `index.ts` exits with a clear error if credentials are not configured.
...
- In unit tests that exercise foreman logic without Supabase, call `setupInMemoryTasks(taskManager?)` from `tests/helpers/task.ts` in `beforeEach`. This spies on all `Task` statics and instance methods with an in-memory `Map` backing. Mutations emit on `Task.events` (and, if a `TaskManager` is passed, directly on it too). Do not call `initDb()` in unit tests — `setupInMemoryTasks` handles mocking at the `Task` layer directly.
