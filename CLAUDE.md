...
- **GitHub token auth** (preferred): if `worker_hello` carries a `githubToken` and the App is configured (`appId` + `appPrivateKey`), the foreman checks for a repo installation. If the repo has no `installationId`, the foreman checks for an org-level `Installation` whose `accountLogin` matches the repo owner (lazy activation for org installs). If one is found, the repo is linked, activated, and tasks are seeded before auth proceeds. If no installation exists at any level, the foreman sends a fatal `foreman_error` directing the user to install the App. If an installation is found, the foreman calls `GET /user` with the worker token to get the login, then `GET /repos/.../collaborators/{login}/permission` with a minted installation token. Workers without `push` or `admin` receive a fatal `foreman_error`.
...
The `/worker:resume <prefix>` command lets a new worker process take over a dead worker's identity. It finds the workspace directory whose name starts with `prefix` (error if none or ambiguous), sets `agentStatus.agentId` to the dead worker's ID, attaches to the existing workspace directory (validates `.git`, re-applies auth, writes lockfile), and connects with `status: "resume"`. The foreman validates: fatal error if the worker is currently connected, if no DB record exists, or if the repo doesn't match. If the dead worker had an active task, the foreman sends both `hello_ack { assigned }` and `task_assigned` so the new process resumes normally. If no active task, it sends `hello_ack { ready }`. The worker also attempts to locate the dead worker's last Claude session ID (newest `.jsonl` in `~/.claude/projects/`) so conversation history is preserved.
...
- `npm run test:worker-exit` — integration test verifying the worker process exits cleanly after `/exit`
- `npm run test:browser` — Playwright browser tests for the admin dashboard (requires `npm run build` first)
- `npm run lint` — ESLint
- `npx tsc --noEmit` — type check

All six run in CI on every PR (coverage, frontend tests, browser tests, lint, type check, worker-exit).
...
- **Verbose output**: gate detailed output (full paths, URLs, diagnostic info) behind `settings.verbose` (read from the `Settings` model, which is seeded from `config.verbose` at startup). Controllers that need it accept `{ verbose: boolean }` — `Settings` satisfies this interface since it has a `verbose` getter, and reading from listeners fires at event time so runtime changes apply immediately. Never print credentials or tokens even in verbose mode; mask them (e.g. `[token]`) in any displayed URL.
...
- **Keyboard rules**: `^C`/`^D` at a visible prompt (states 1, 3-at-prompt, or 2-reserved) = quit — calls `stop()` (with task-quit confirmation if active) then attempts exit on `^D`/`/exit`. Exit may be cancelled: if a workspace exists with uncommitted changes, the user is prompted; choosing "No, cancel" returns to the REPL prompt without exiting. `^C` with no visible prompt (state 3 running a query) = interrupt. `^C` with no visible prompt (state 2 waiting, `ask("")`) = reserve — calls `workerController.reserve()`, sends `worker_reserved` to foreman, transitions to a visible prompt so the user can `/worker:claim`. `^D` with no visible prompt (state 2, `ask("")`) = no-op.
