...
- **`src/foreman/memory-db.ts`** — `createMemoryTaskDb()`: in-memory Supabase shim used when Supabase is not configured (local dev without `.env`, smoke tests, browser tests). Implements the query patterns that `Task` uses, backed by an in-memory `Map`.
