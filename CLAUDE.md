...
- **`src/foreman/models/task.ts`** — `Task` active-record class. Owns all DB reads/writes for the `tasks` table. Must be initialised once via `initTask(supabase, onTaskChanged?)` before use. Static finders: `Task.get()`, `Task.getByIssue()`, `Task.getByPr()`, `Task.getByWorker()`, `Task.list()`, `Task.upsert()`. Instance mutations: `assign()`, `complete()`, `revert()`, `close()`, `reopen()`, `registerPr()`, `unregisterPr()`, `mergePr()`, `updateContent()`, `delete()`. The `status` getter derives the current status from timestamps. `toSnapshot(graph, openIssues)` returns a `TaskSnapshot` with blocked status overlaid. Use `Task.fromTest()` in tests to create instances without DB. Every mutation automatically calls the `onTaskChanged` callback.
- **`src/foreman/models/task-model.ts`** — `TaskManager` class. Owns only ephemeral in-memory state with no DB backing: event queues (buffered GitHub events for pending/disconnected workers), branch mappings (branch→taskId for routing check_run/check_suite events), and GitHub issue tracking (`labeledIssues`/`openIssues` for dependency resolution). All DB reads go through `Task` statics. Emits `"changed"` (wired from `initTask`'s `onTaskChanged`) so the admin dashboard can refresh. Provides startup methods: `loadActiveTasksFromDb()` and `loadIssuesFromGithub()`.
...
- `Task` (active-record in `task.ts`) owns all DB reads/writes. `TaskManager` (`task-model.ts`) owns only ephemeral state (event queues, branch maps, issue tracking). **There is no in-memory cache — all reads go through `Task` statics.** `initTask(supabase, onTaskChanged?)` must be called once at startup to wire the DB. Supabase is required to run the foreman; `index.ts` exits with a clear error if credentials are not configured.
...
- **Status is derived** via the `Task.status` getter based on priorities: `completedAt` → 'complete', `issueClosedAt` → 'closed', `prMergedAt` → 'merged', `workerId` → 'assigned', `prNumber` → 'pushed', `isBlocked()` → 'blocked', else → 'pending'.
- `Task.upsert()` conflicts on `task_id`. If an issue is re-labeled `brunel:ready` (e.g. after completing), it resets status fields and refreshes `title`, `body`, `labels` — it does **not** ignore duplicates. Returns the `Task` instance.
...
- On foreman restart, `taskManager.loadActiveTasksFromDb()` restores ephemeral branch mappings from DB, then `taskManager.loadIssuesFromGithub()` fetches open issues with the task label and reconciles blocked/unblocked state. Tasks with `completedAt` set are skipped by startup. When a worker reconnects with a busy hello for an unknown task, the foreman creates a placeholder task via `Task.upsert()` so the worker can complete normally.
- `reconcile()` only removes **pending** and **blocked** tasks that are no longer in `labeledIssues`. Assigned and complete tasks are never removed by reconcile. Removal calls `task.delete()`, which deletes the DB row — but only if `assigned_at IS NULL`. `task.revert()` (called on `worker_goodbye`) preserves `assigned_at`, so rows that ever had a worker survive reconcile even after reverting to pending.
- `labeledIssues` and `openIssues` are owned by `TaskManager`. Use `taskManager.trackIssue()`, `taskManager.untrackIssue()`, `taskManager.closeIssue()` etc. to mutate them. Tests inject state via these methods — not by passing raw maps to `createForemanWss`.
...
- In unit tests that exercise foreman logic without Supabase, call `setupInMemoryTasks(taskManager?)` from `tests/helpers/task.ts` in `beforeEach`. This spies on all `Task` statics and instance methods with an in-memory `Map` backing, and optionally wires mutations to emit `"changed"` on the provided `TaskManager`. Do not call `initTask()` in unit tests — `setupInMemoryTasks` handles mocking at the `Task` layer directly.
- For integration-level tests that spin up a real foreman process in-process (e.g. browser tests in `tests/browser/server.ts`), call `initTask(createMemoryTaskDb(), ...)` from `tests/helpers/memory-db.ts`. The smoke test uses a real Supabase instance (started by CI) rather than the in-memory shim.
