...
- **`src/foreman/index.ts`** — Entry point and orchestrator. Wires components together, starts HTTP + WSS servers. Also re-exports key symbols for convenience (`WorkerRegistry`, `createForemanWss`, `createHttpServer`, `summaryEvent`, `isMutedEvent`, `TaskModel`, `Task`).
- **`src/foreman/task-model.ts`** — `TaskModel` class: the single entry point for all task state. Encapsulates paired in-memory (`TaskQueue`) + persistent (`TaskStore`/Supabase) updates so every state transition touches both stores atomically. Also owns `labeledIssues`/`openIssues`.
...
- **`src/foreman/wss.ts`** — `createForemanWss()`: WebSocket plumbing + message dispatch. Creates the `EventRouterDeps` and delegates routing to the event router. Contains connection lifecycle handlers (`handleWorkerHello`, `handleTaskComplete`, `handleWorkerGoodbye`) and assignment logic.
...
- **`src/foreman/event-fmt.ts`** — `fmtEvent` and `fmtEventDetails` for formatting GitHub events into human-readable summaries. Foreman-side copy so the foreman module has zero imports from `display.ts`.
