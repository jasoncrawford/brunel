...
- **Models** (`models/`) — `workspace.ts` (git/npm workspace management; extends `EventEmitter`, emits `"clone-start"`, `"reset-start"`, `"reset-retry"`, `"reset-reclone"`, `"npm-install"`, `"destroy"` events instead of calling `print()` directly — callers subscribe in `startWorkerMode()`), `settings.ts` (pure model for runtime-settable preferences; exports `Settings` class — owns model and effort state, emits `"change"` on updates via `_setModel`/`_setEffort`; also exports effort/model constants, types, and model-cache helpers — no interactive methods), `query-stats.ts` (tracks token usage and turn counts; exports `WORKING_VERBS`, `pickWorkingVerb`, and `QueryStats` class — no view imports)
- **Views** (`views/`) — `display.ts` (TUI rendering; exports `Display` class with `BrunelConfig` injected in constructor — class methods use `this.config.verbose`; also exports `WorkerDisplay` interface `{ print, printForemanMessage }` satisfied by `Display`; all pure utilities like `c`, `s`, `resolve`, `fmtEvent`, etc.; no singletons or standalone delegates), `status-bar.ts` (status bar state and rendering; exports `StatusBar` class — holds all worker status state — and a shared `statusBar` singleton initialized via `initStatusBar()` at startup), `input.ts` (readline-based REPL input: `ask`, `pick`, `pickMultiple`, `pickQuestion`)
- **Controllers** (`controllers/`) — `worker-controller.ts` (WS protocol + task lifecycle; owns `WorkerSession`, `generateAgentId`, `WORKER_NAMES`, `startWorkerMode(display)`, `registerWorkerCommands(session, registry, display)`, `confirmTaskQuit(info, display, pickFn?)`), `workspace-controller.ts` (registers `/workspace:*` slash commands; `registerWorkspaceCommands(workspace, registry, display)` takes injected `display: WorkerDisplay`), `command-controller.ts` (`CommandController` class — instantiated in `index.ts` and injected into all modules; supports scoped sub-registries via `scoped(prefix)`; `registry.listCommandNames()` and `registry.listCommands()` return all available commands including file-based commands and skills), `settings-controller.ts` (`SettingsController` class — wraps `Settings` model with `display: WorkerDisplay`; exposes `pickModel(args, pickFn, fetchModelsFn)` and `pickEffort(args, pickFn)`)
