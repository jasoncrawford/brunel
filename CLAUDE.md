...
Other key files: `index.ts` (composition root + routing loop; the startup block is the single composition root — it constructs the full object graph in order: `settings` → `statusBar` → `display` → `input` → `picker` → `agentController` → `confirm` → `workspace` → `workspaceController` → `startWorkerMode` (if worker mode) → `fetchModelsFn` → `settingsController` → `registry` → `controller`, then calls `main()`; exports `main(agentController, permConfig, display, settings, input, picker, settingsController, fetchModelsFn, controller, session, workspaceController, workerCleanup)` which registers commands (some handlers close over loop-local `sessionId` and `doExit`, so registration stays in `main()`) and runs the routing loop — session events (`"prompts_ready"`, `"fatal"`) and user input from `ask()` both push to a shared async event channel; the routing loop sleeps on `nextRoutingEvent()` and dispatches whatever arrives; session event handlers call `input.cancel()` before pushing so the terminal is released; `hasPendingPrompts()` at the top of each iteration handles lost wakeups (events that fired before `listenForInput()` was called)), `worker-prompts.ts` (prompt templates).
