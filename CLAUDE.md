...
- **`src/agent/commands.ts`** — Command registry: `register(name, opts)`, `lookup(name)`, `listAll(workerMode)`, `execute(name, args)`, `_reset()`. Each command is registered with its implementation handler (`CommandHandler`). No built-in registrations at module load — commands are registered at startup by their owning modules. `HandlerResult = void | "exit" | "task-complete"` propagates signals through `execute()`. `_reset()` clears the registry for test isolation.
...
- **`src/agent/workspace.ts`** — Git/npm workspace management: `Workspace` class, branch checkout, npm install, safety confirmations. Also exports `registerWorkspaceCommands(namespace, deps, workerMode?)` which registers `workspace:create/reset/remove/prune` with their implementations. `WorkspaceCommandDeps` uses getters/setters for mutable state so commands close over live references.
